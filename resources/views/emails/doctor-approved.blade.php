...
<body>
    <div class="container">
        ...
        <div class="content">
            <h2>Welcome to AAROGYA, Dr. {{ $doctor->name }}! 👋</h2>

            <p>We are pleased to inform you that your account has been <strong>verified and approved</strong>.</p>

            <div style="background: #ecfdf5; padding: 20px; border-radius: 12px; margin: 20px 0; border-left: 4px solid #22c55e;">
                <p style="margin: 0;"><span class="badge-verified">✓ Verified Doctor</span></p>
                <p style="margin-top: 10px; color: #64748b;">You can now start accepting appointments from patients.</p>
            </div>

            <p><strong>Your Profile Details:</strong></p>
            <ul style="color: #334155; line-height: 1.8;">
                <li><strong>Name:</strong> Dr. {{ $doctor->name }}</li>
                <li><strong>Specialization:</strong> {{ $doctor->specialization }}</li>
                <li><strong>NMC Registration:</strong> {{ $doctor->nmc_registration }}</li>
                @if($doctor->clinic)
                    <li><strong>Clinic:</strong> {{ $doctor->clinic->name }}</li>
                @endif
            </ul>
